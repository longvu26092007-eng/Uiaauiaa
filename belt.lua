...
task.spawn(function()
    ...
    while task.wait(4) do
        ...
        if invValid then
            ...
            -- 4. LUỒNG FARM BELT & EGG
            if hasBlack then
                ClearBlackBeltFailed()

                if IsLearnDone() then
                    -- 4/4 Dragon Egg là HẾT
                    if eggCount < 4 then
                        ...
                        ActionStatus.Text = "Hành động: Đã đủ 4/4 Dragon Egg! HOÀN THÀNH."
                    ...
                    if boneCount >= 3 then
                        ...
                        ActionStatus.Text = "Hành động: Đủ Đai & Xương! Đi học Tether..."
                        ...
                        if arrived then
                            ...
                            if ok1 and RF1 then
                                pcall(function() RF1:InvokeServer({[1]={NPC="Dragon Wizard", Command="Speak"}}) end)
                                task.wait(3)
                                local res = RF1:InvokeServer({[1]={NPC="Dragon Wizard", Command="LearnTether"}})
                                if res ~= false then
                                    SaveLearnStatus()
                                    CURRENT_STATE = "UNKNOWN"
                                end
